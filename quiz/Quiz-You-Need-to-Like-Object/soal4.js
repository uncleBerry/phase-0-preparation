...
function travelingIndonesia(arr, emoney) {
  ...
  let biayaTransportasi = {Pesawat: 275000, Kereta: 250000, Bis: 225000};

  let diskon = {OVO: 0.15, Dana: 0.10, Gopay: 0.05};
  ...
  for(let i = 0; i < arr.length; i++) {
    ...
    person.push(temp);
    dataTraveler.push(person);
  }



  for(let i = 0; i < dataTraveler.length; i++) {
    let nama = dataTraveler[i][0];
    let asal = dataTraveler[i][1];
    let tujuan = dataTraveler[i][2];
    let transport = dataTraveler[i][3];

    let indexAsal = -1;
    let indexTujuan = -1;

    for(let j = 0; j < rute.length; j++) {
      if(asal === rute[j]) indexAsal = j;
      else if(tujuan === rute[j]) indexTujuan = j;
    }

    let jarakTempuh = Math.abs(indexAsal - indexTujuan);
    let biayaDasar = 0;
    let potongan = 0;

    if(biayaTransportasi[transport]) {
      biayaDasar = jarakTempuh * biayaTransportasi[transport];
    }

    if(diskon[emoney]) {
      potongan = biayaDasar * diskon[emoney];
    } 

    let finalCost = biayaDasar - potongan;

    result.push({
        name: nama,
        departureCity: asal,
        destinationCity: tujuan,
        transport: transport, // Jangan lupa ini diminta di output
        totalCost: finalCost
    });
  }
